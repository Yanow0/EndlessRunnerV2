...
void GraphicObstacle::afficherObstacle(Jeu jeu) {
    ...
    for (int k=0; k<obstacle.nbObstacles(); k++) {
        SDL_Rect image;
        image.x= obstacle.getObstacle(k).pos->getX()*20;
        image.y= obstacle.getObstacle(k).pos->getY()*20;
        image.w= obstacle.getObstacle(k).taille->getLargeur()*20;
        image.h= obstacle.getObstacle(k).taille->getHauteur()*20;
    ...
    cout << obstacle.nbObstacles() << " obstacles" << endl;
